    ...
    <header className="fixed inset-x-0 top-0 z-50">
      <div className="mx-auto max-w-7xl px-4 pt-4 md:px-8 md:pt-5">
        ...
        >
          <div className="flex items-center justify-between">
            <Link href="/" onClick={closeMenu} className="group flex min-w-0 flex-col leading-none">
                ...
                return (
                  <Link
                    ...
                    className={`nav-link text-sm font-medium tracking-[0.08em] transition-colors ${
                      active ? "text-honey" : "text-cream/85 hover:text-parchment"
                    }`}
                  >
                    {link.label}
                  </Link>
            ...
            <div className="hidden items-center gap-4 md:flex">
              <a href={site.phoneHref} className="text-sm font-medium text-cream/80 transition-colors hover:text-honey">
                {site.phone}
              </a>
      ...
      >
            ...
            return (
              <Link
                ...
                className={`heading-display animate-reveal rounded-2xl px-4 py-3 text-4xl transition-colors ${
                  active ? "bg-[rgba(185,133,75,0.15)] text-honey" : "text-parchment/90 hover:text-honey"
                }`}
        ...
        <div className="mt-10 rounded-2xl border border-[rgba(242,224,194,0.2)] bg-[rgba(255,255,255,0.03)] p-4">
          <p className="text-xs uppercase tracking-[0.2em] text-latte">Contact</p>
          <a href={site.phoneHref} className="mt-3 block text-sm text-cream transition-colors hover:text-honey">
            {site.phone}
          </a>
          <a href={`mailto:${site.email}`} className="mt-1 block text-sm text-cream transition-colors hover:text-honey">
            {site.email}
          </a>
          <p className="mt-3 text-xs text-stone">Open daily from 7:00 AM to 2:00 PM</p>
        </div>
      </aside>
    </header>
  );
